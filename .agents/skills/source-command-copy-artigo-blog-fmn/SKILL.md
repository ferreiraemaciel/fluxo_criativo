---
name: "source-command-copy-artigo-blog-fmn"
...
# source-command-copy-artigo-blog-fmn

Use this skill when the user asks to run the migrated source command `copy-artigo-blog-fmn`.
...
2. `.Codex/rules/copy/checklist-light-copy.md` (12 proibições).
3. `.Codex/skills/revisora/references/manual-copy.md` (+ `phrases.md`, `structures.md` se precisar).
...
Segue a regra global do projeto: gerar imagem sempre via ChatGPT (skill `gerar-imagem`, usa Codex in Chrome), nunca gerar imagem por outro meio se o Chrome estiver disponível.
