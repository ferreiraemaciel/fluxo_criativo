---
name: "source-command-criativo-estatico-criativo-historia"
description: "Migrated source command `criativo-estatico-criativo-historia`"
---

# source-command-criativo-estatico-criativo-historia

Use this skill when the user asks to run the migrated source command `criativo-estatico-criativo-historia`.
...
Aplicar os princípios do Manual da Copy (`.Codex/skills/revisora/references/manual-copy.md`):
...
Antes de mostrar a legenda ao aluno: aplicar a rotina de auto-revisão de copy do AGENTS.md (carregar Manual da Copy + acionar a skill `revisora`) na legenda. Corrigir direto. Nunca mostrar versão bruta.
...
Use o comando Python correto da sessão (`python3` ou `py -3`), conforme a seção Execução de Scripts Python do AGENTS.md.
...
Apresente o resultado conforme o modo escolhido. Sempre mostrar o caminho absoluto (regra 4a do AGENTS.md).
...
Toda legenda pro Instagram entregue por esta skill deve ter **no mínimo 3 linhas**, redigida seguindo os princípios do Manual da Copy (`.Codex/skills/revisora/references/manual-copy.md`):
...
- Aprovação obrigatória antes de salvar (regra 5 do AGENTS.md). Pular só se o aluno pediu explicitamente "ir direto à versão final" na mesma sessão.
...
- Após salvar, sempre exibir caminho absoluto no chat (regra 4a do AGENTS.md).
