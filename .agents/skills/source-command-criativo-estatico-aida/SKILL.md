---
name: "source-command-criativo-estatico-aida"
description: "Migrated source command `criativo-estatico-aida`"
---

# source-command-criativo-estatico-aida

Use this skill when the user asks to run the migrated source command `criativo-estatico-aida`.
...
Cruze com a referência interna em `.Codex/skills/anuncios/references/formatos-virais-instagram.md`.
...
**Antes de mostrar as versões:** aplicar a rotina de auto-revisão de copy obrigatória do AGENTS.md (carregar Manual da Copy + acionar revisora) e corrigir direto no texto. Não mostrar versão bruta.
...
Use o comando Python correto da sessão (`python3` ou `py -3`), conforme a seção Execução de Scripts Python do AGENTS.md. Nos comandos abaixo, troque `py -3` pelo comando detectado se necessário.
