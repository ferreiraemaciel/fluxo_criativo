  ...
  try {
    ...
    if (body.completou_quiz && body.whatsapp && funnelSlug === "fotografo-protegido") {
      const { data: leadAtual } = await sb
        ...
        .eq("code", body.code)
        .single();
      if (!leadAtual?.whatsapp_resultado_enviado && !leadAtual?.resultado_agendado_para) {
        const agendadoPara = new Date(Date.now() + 5 * 60 * 1000).toISOString();
        await sb.from("quiz_leads").update({ resultado_agendado_para: agendadoPara })
          .eq("funnel_slug", funnelSlug).eq("code", body.code);
      }
    }
