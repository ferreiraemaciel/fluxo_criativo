-- Migration 025: gera o insight do dia (rotação de achados reais) + agenda diária via pg_cron
...
begin
  ...
  insert into quiz_insights(dia,titulo,gancho,detalhe,formato) values (current_date,titulo,gancho,detalhe,'Reels')
  on conflict (dia) do update set titulo=excluded.titulo, gancho=excluded.gancho, detalhe=excluded.detalhe;
end $$;

create extension if not exists pg_cron;
select cron.schedule('insight-quiz-diario','0 9 * * *','select gerar_insight_quiz();');
