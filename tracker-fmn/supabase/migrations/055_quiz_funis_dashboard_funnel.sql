-- Migration 055: adiciona filtro por funil (funnel_slug) ao quiz_funis_dashboard.
-- A função passa de 2 para 3 parâmetros. p_funnel null = todos os funis (comportamento anterior).
-- É necessário dropar e recriar porque PostgreSQL não permite alterar assinatura via CREATE OR REPLACE.

drop function if exists quiz_funis_dashboard(date, date);

create function quiz_funis_dashboard(
...
as $$
  with base as materialized (
    select created_at, completou_lead, area_atuacao, profissionalizacao, tipo_negocio,
           ...
           quer_modelos, utm_campaign, utm_content, device_platform, nivel_risco
    from quiz_leads
    where (p_from   is null or created_at >= p_from)
      and (p_to     is null or created_at <  (p_to + 1))
      and (p_funnel is null or funnel_slug = p_funnel)
  ),
  prev as (
  ...
  select jsonb_build_object(
    ...
    'criativo_qualidade', (select jsonb_agg(jsonb_build_object('criativo',criativo,'leads',leads,'alto',alto) order by leads desc) from (
        select split_part(coalesce(nullif(utm_content,''), '(sem criativo)'), '|', 1) criativo,
               count(*) filter (where completou_lead) leads,
               count(*) filter (where completou_lead and nivel_risco in ('Alto','Altíssimo')) alto
        from base where utm_content is not null group by 1
        having count(*) filter (where completou_lead) > 0 order by 2 desc limit 10) d)
  );
$$;
