-- Migration 021: performance. O 'base' agora seleciona só as colunas usadas
-- nos gráficos (sem os jsonb pesados raw/respostas), cortando o tempo de ~6s
-- para fração de segundo. Mantém timeout de segurança da função.
...
as $$
  ...
  select jsonb_build_object(
    'kpis', (select jsonb_build_object(
        ...
        'periodo_min', min(created_at), 'periodo_max', max(created_at)) from base),
    ...
    'por_mes', (select jsonb_agg(jsonb_build_object('mes', mes, 'leads', leads, 'com_email', com_email) order by mes) from (
        select to_char(date_trunc('month', created_at),'YYYY-MM') mes, count(*) leads, count(*) filter (where completou_lead) com_email
        from base where created_at is not null group by 1) m),
