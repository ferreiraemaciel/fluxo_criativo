...
```
python3 .claude/skills/corte-premiere/scripts/inspecionar.py "CAMINHO/Sequencia.xml"
```

Passo 2. **Ler a transcrição** com timecode (e, se o objetivo pedir, varrer
menções sensíveis, por exemplo marcas concorrentes):
```
python3 .claude/skills/corte-premiere/scripts/transcricao.py --json "CAMINHO/transcricao.json" --scan "volvo,mercedes,man,iveco,daf"
```
...
```
python3 .claude/skills/corte-premiere/scripts/conformar.py --edl "CAMINHO/Objetos/edl_projeto.json"
```

Passo 5. **Validar** antes de entregar:
```
python3 .claude/skills/corte-premiere/scripts/validar.py --xml "CAMINHO/CORTE.xml" --source "CAMINHO/Sequencia.xml"
```
