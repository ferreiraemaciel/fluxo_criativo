...
**Subir (push): automático, após aprovação, sem precisar pedir de novo.** Sempre que Felipe OU Amanda aprovar uma entrega ou mudança que toque um arquivo dentro de um desses repositórios, faça commit com mensagem curta e descritiva e `git push` pro `origin` (nunca pro `mentoria`) logo em seguida, sem perguntar "posso subir pro GitHub" a cada vez, isso já está autorizado pros dois igualmente.
